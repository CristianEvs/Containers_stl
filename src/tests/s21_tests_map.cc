...
TEST(map, ConstructorInitializerMap) {
    ...
    for (auto my_it = my_map.begin(); my_it != my_map.end(); ++my_it, ++orig_it) {
...
for (; my_it != my_map.end(); ++my_it, ++orig_it) {
    ...
    EXPECT_EQ((*my_it).second, (*orig_it).second);
}
}
...
for (; my_it != my_map.end(); ++my_it, ++orig_it) {
    ...
    auto item1 = my_map_merge.contains(4);
    auto item2 = my_map_merge.contains(3);
    EXPECT_TRUE(item1);
    EXPECT_FALSE(item2);
}
